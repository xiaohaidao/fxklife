...
public:
  HRESULT CopyData(BYTE *pData, UINT32 NumFrames, BOOL *pDone,
                   WAVEFORMATEX *pwfx, HMMIO hFile);
};

HRESULT WriteWaveHeader(HMMIO hFile, LPCWAVEFORMATEX pwfx, MMCKINFO *pckRIFF,
                        MMCKINFO *pckData);
...
int main() {
  ...
  hFile = mmioOpen(
      ...
      fileName, &mi, MMIO_WRITE | MMIO_CREATE);

  if (NULL == hFile) {
    printf("mmioOpen(\"%ls\", ...) failed. wErrorRet == %u", fileName,
           GetLastError());
    return E_FAIL;
  }
...
#pragma prefast(                                                               \
  ...
  if (lBytesToWrite != lBytesWritten) {
    wprintf(L"mmioWrite wrote %u bytes : expected %u bytes", lBytesWritten,
            lBytesToWrite);
    return E_UNEXPECTED;
  }
  ...
  if (clock() >
      10 * CLOCKS_PER_SEC) // Record 10 seconds. From the first time call
                           // clock() at the beginning of the main().
    *pDone = true;
...
HRESULT RecordAudioStream(MyAudioSink *pMySink) {
  ...
  while (bDone == FALSE) {
    ...
    while (packetLength != 0) {
      ...
      hr = pMySink->CopyData(pData, numFramesAvailable, &bDone, pwfx,
                             (HMMIO)hFile);
      EXIT_ON_ERROR(hr)
...
HRESULT WriteWaveHeader(HMMIO hFile, LPCWAVEFORMATEX pwfx, MMCKINFO *pckRIFF,
  ...
  LONG lBytesWritten = mmioWrite(
      hFile, reinterpret_cast<PCHAR>(const_cast<LPWAVEFORMATEX>(pwfx)),
      lBytesInWfx);
  ...
  DWORD frames = 0;
  lBytesWritten =
      mmioWrite(hFile, reinterpret_cast<PCHAR>(&frames), sizeof(frames));
